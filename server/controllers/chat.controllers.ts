...
const createOrGetAOneOnOneChat = asyncHandler(async (req, res) => {
  ...
  payload?.participants?.forEach(
    (participant: { _id: { toString: () => string } }) => {
      if (!req.user) {
        return res
          .status(400)
...
const deleteOneOnOneChat = asyncHandler(async (req, res) => {
  ...
  const otherParticipant = payload?.participants?.find((participant: any) => {
    if (!req.user) {
      return res.status(400).json({ message: "user not found", success: true });
    }
    return participant?._id.toString() !== req.user._id.toString(); // get the other participant in chat for socket
  });
...
const addNewParticipantInGroupChat = asyncHandler(async (req, res) => {
  ...
  if (
    existingParticipants?.includes(new mongoose.Types.ObjectId(participantId))
  ) {
